...
if($count==0){
$serial=$_POST[$key];
$sql = sprintf("SELECT serial,count FROM wpf9e767tsuri6th WHERE serial='%s' and trust_id='%d'", $serial, $this->id);
$query_serial = $wpdb->get_results($sql);
if(!$query_serial){
echo "<p>シリアル番号【".$serial."】は間違っています。大文字、小文字などに間違いがないかご確認ください。</br>5秒後に自動的に入力フォームに遷移します。</p>";
...
if($qserial->count==1)
{
echo "<p>シリアル番号【".$qserial->serial."】は既に応募済みです。再度シリアル番号をお確かめください。</br>5秒後に自動的に入力フォームに遷移します。</p>";
echo "<meta http-equiv='refresh' content='5;URL='". $_SERVER['REQUEST_URI'] ."'>";
exit;
}else{

//発行するsql文の作成
$sql = sprintf("UPDATE wpf9e767tsuri6th SET count=1 WHERE serial='%s'", $serial);
